...
MesherNaive::CreateVertices(const Block *const *const *const blocks) {
  std::vector<glm::vec3> vertices;
      ...
      for (int z = 0; z < CHUNK_SIZE_Z; z++) {
        ...
        for (int currFace = FACE_RIGHT; currFace < NUM_FACES; currFace++) {
          ...
          for (int currCoord = COORDINATE_X; currCoord < NUM_COORDINATES;
            ...
            // Which chunk-axis (or "coordinate") boundary are we checking
            // for? Ex. if we're checking the X coordinate boundary, then we
            // set "size" to the span of the chunk X length. Later on, we use
            // "size" to determine if we're lying on the edge of a chunk.
            int size = 0;
            if (currCoord == COORDINATE_X)
              size = CHUNK_SIZE_X;
            else if (currCoord == COORDINATE_Y)
            ...
            if (index < 0 || index == size) {
              ...
              // Since we found the out of bounds index, we can skip the rest
              // of the check
              break;
            }
          }
